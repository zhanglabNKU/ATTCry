...
from keras.layers import Dense, Dropout, Flatten, Input, Softmax, Lambda, concatenate, LSTM, add, GRU
...
test_index = np.arange(0, len(test_data))
...
fit = model.fit(x_train, y_train, validation_data=(x_test, y_test), nb_epoch=100, class_weight=class_weight_dict,
                batch_size=batch_size, shuffle=True, callbacks=[early_stopping, tbCallBack])
y_pred_net = model.predict(x_test)
auc = metrics.roc_auc_score(y_test, y_pred_net)
